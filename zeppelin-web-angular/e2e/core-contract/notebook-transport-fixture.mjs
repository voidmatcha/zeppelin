...
function validateExecutionCoveredOperations(errors, fixture) {
  if (fixture.metadata?.provenance?.captureMode !== 'execution') return;
...
export function validateCaptureProvenance(metadata) {
  ...
  if (!/^[0-9a-f]{40}$/.test(provenance.baseCommit ?? '')) {
    errors.push('metadata.provenance.baseCommit must be a full Git commit hash');
  }
  if (provenance.sourceCommit !== provenance.baseCommit) {
    errors.push('metadata.provenance.sourceCommit must equal baseCommit');
  }
  validateBuildManifest(errors, provenance);
...
const manifestArtifactKey = artifact => `${artifact.path}\0${artifact.selection}`;

function validateBuildManifest(errors, provenance) {
  const prefix = 'metadata.provenance.buildManifest';
  const buildManifest = provenance.buildManifest;
  if (!buildManifest || typeof buildManifest !== 'object' || Array.isArray(buildManifest)) {
    errors.push(`${prefix} is required`);
    return;
  }
  if (!sha256Pattern.test(buildManifest.id ?? '')) {
    errors.push(`${prefix}.id must be a SHA-256 digest`);
  } else {
    const manifestBody = {
    ...
    const computedId = createHash('sha256')
      ...
      .digest('hex');
    if (buildManifest.id !== computedId) {
      errors.push(`${prefix}.id must match the manifest content`);
    }
  }
  if (buildManifest.sourceCommit !== provenance.sourceCommit) {
    errors.push(`${prefix}.sourceCommit must match metadata.provenance.sourceCommit`);
  }
  if (buildManifest.baseCommit !== provenance.baseCommit) {
    errors.push(`${prefix}.baseCommit must match metadata.provenance.baseCommit`);
  }
  if (buildManifest.version !== 3) {
    errors.push(`${prefix}.version must be 3`);
  }
  for (const target of ['angularWeb', 'classicWeb']) {
    if (!['directory', 'war', 'missing'].includes(buildManifest.launchTargets?.[target]?.kind)) {
      errors.push(`${prefix}.launchTargets.${target}.kind must be directory, war or missing`);
    }
  }
  const sourceTree = buildManifest.sourceTree;
  if (!Number.isInteger(sourceTree?.fileCount) || sourceTree.fileCount < 1) {
    errors.push(`${prefix}.sourceTree.fileCount must be a positive integer`);
  }
  if (!sha256Pattern.test(sourceTree?.sha256 ?? '')) {
    errors.push(`${prefix}.sourceTree.sha256 must be a SHA-256 digest`);
  }
  ...
  for (const [index, artifact] of artifacts.entries()) {
    ...
    if (
      ...
      typeof artifact.selection !== 'string'
    ) {
      errors.push(`${artifactPrefix} must be an object with string path and selection`);
      artifactShapesValid = false;
      continue;
    }
    if (typeof artifact.exists !== 'boolean') {
      errors.push(`${artifactPrefix}.exists must be a boolean`);
    }
    if (!Number.isInteger(artifact.fileCount) || artifact.fileCount < 0) {
      errors.push(`${artifactPrefix}.fileCount must be a non-negative integer`);
    } else if (artifact.exists === false && artifact.fileCount !== 0) {
      errors.push(`${artifactPrefix}.fileCount must be 0 for a missing artifact`);
    }
    if (!sha256Pattern.test(artifact.sha256 ?? '')) {
      errors.push(`${artifactPrefix}.sha256 must be a SHA-256 digest`);
    }
  }
  if (
    artifactShapesValid &&
    stableJson(artifacts.map(manifestArtifactKey).sort()) !==
      stableJson(expectedManifestArtifacts.map(manifestArtifactKey).sort())
  ) {
    errors.push(`${prefix}.artifacts must list exactly the launcher build outputs`);
  }
}
