...
test.describe('Notebook editor search', () => {
  ...
  test('updates the match count after a single replace', async ({ page }) => {
    ...
    // Activate the current match in the editor first, then replace it: Monaco's single
    // replace is a no-op until a match is the live selection, not just the shown "1 of 3".
    await editorSearchPage.findInput.press('Enter');
