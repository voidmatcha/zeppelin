...
test.describe('Collaborative mode', () => {
  ...
  test('toggles between personal and collaboration mode from the action bar', async ({ page }) => {
    ...
    // Open the single-paragraph route: it skips the main route's unset->'false'
    // normalization, so an unset personalizedMode reaches the toggle unchanged — the
    // only route where the no-op bug reproduces (and thus where this pins the fix).
    const { noteId, paragraphId } = await createTestNotebook(page);
    await collaborationPage.openParagraph(noteId, paragraphId);
