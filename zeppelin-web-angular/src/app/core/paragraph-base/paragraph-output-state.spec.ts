...
describe('ParagraphOutputState', () => {
  ...
  it('discards untyped pending appends when manually clearing output', () => {
    const state = new ParagraphOutputState();
    state.reset([{ type: DatasetType.TEXT, data: 'old' }]);
    state.append(1, 'discarded pending output');

    state.clearPreservingTypes();
    state.update(1, DatasetType.TABLE, 'new table');
    expect(state.snapshot()).toEqual([
      { type: DatasetType.TEXT, data: '' },
      { type: DatasetType.TABLE, data: 'new table' }
    ]);
    expect(state.isHidden(0)).toBe(true);
    expect(state.isHidden(1)).toBe(false);
    state.finish([{ type: DatasetType.TEXT, data: 'final' }]);
    expect(state.isHidden(0)).toBe(false);
    expect(state.append(0, 'late')).toBeUndefined();
  });
