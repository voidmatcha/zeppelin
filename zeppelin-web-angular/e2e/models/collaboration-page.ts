...
export class CollaborationPage extends BasePage {
  ...
  // The single-paragraph route skips the main route's unset->'false' normalization
  // (notebook.component.ts), so an unset personalizedMode reaches the action bar as-is.
  // The toggle bug only reproduces here, so the toggle test must use this route.
  async openParagraph(noteId: string, paragraphId: string): Promise<void> {
  ...
  async confirmPersonalizedModeChange(): Promise<void> {
    // Scope to this confirm dialog and wait for it to close: the global OK locator
    // could hit another modal, and a back-to-back toggle could race the closing animation.
