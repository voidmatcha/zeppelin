...
export class NotebookKeyboardPage extends BasePage {
  ...
  async setCodeEditorContent(content: string, paragraphIndex: number = 0): Promise<void> {
    ...
    // The note's initial paragraph text can arrive over the websocket after the clear
    // and merge with the fill (seen as a doubled "%python" prefix on Firefox); the
    // outer retry re-clears and re-seeds when that one-time race lands.
