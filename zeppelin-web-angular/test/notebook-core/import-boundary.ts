...
const isReactNotebookCoreProofModule = (path: string): boolean =>
  path.startsWith(`${reactNotebookCoreProofRoot}/`) && !path.includes('/node_modules/');

// Any source in the separate-build remote can be exposed, so scan the directory
// instead of one probe. Build output and webpack configuration are not remote code.
...
export const findReactNotebookConsumerViolations = (
  ...
  const collectRemoteModules = (path: string): void => {
    if (remoteModules.has(path) || path.startsWith(sourceRoot) || path.includes('/node_modules/')) {
      return;
    }
    remoteModules.add(path);
    loadModule(path)?.dependencies.forEach(collectRemoteModules);
  };
  // Start from remote sources in the checked program only; package graphs are not remote code.
  ...
  const check = (path: string): void => {
    ...
    // Third-party declarations are only checked for re-exporting the core runtime;
    // consumer transport rules apply to Zeppelin sources, not to package internals.
    violations.push(
