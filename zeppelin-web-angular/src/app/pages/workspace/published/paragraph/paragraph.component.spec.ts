...
it('refreshes React visibility when an empty live slot becomes a terminal result', () => {
  const { component } = setup(true);
  component.getNote(snapshot('note-A', 'RUNNING', ''));
  expect(component.reactProps.hiddenResults).toEqual([true]);
  component.paragraphData({ noteId: 'note-A', paragraph: snapshot('note-A', 'FINISHED', '').note.paragraphs[0] });
