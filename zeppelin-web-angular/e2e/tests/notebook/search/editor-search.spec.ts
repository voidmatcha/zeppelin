...
test.describe('Notebook editor search', () => {
  ...
  test('updates the match count after a single replace', async ({ page }) => {
    const { noteId } = await createTestNotebook(page);
    ...
    // Monaco's single Replace acts only on the match that is the live editor selection.
    // After searchFor the selection is at the line start, so the first click just selects
    // the current match and the second replaces it.
    await editorSearchPage.replaceButton.click();
    await editorSearchPage.replaceButton.click();

    await expect(editorSearchPage.editorText).toContainText('replacement', { timeout: 15000 });
    await expect(editorSearchPage.matchesCount).toContainText(/of 2/, { timeout: 15000 });
  });
