...
PLAYWRIGHT_BASE_URL=http://127.0.0.1:8080 PERF_NOTEBOOK_DISPOSABLE=1 npm run perf:notebook-baseline

# Contract validation only: one cold and one warm sample, plus an unrecorded prime.
PLAYWRIGHT_BASE_URL=http://127.0.0.1:8080 PERF_NOTEBOOK_DISPOSABLE=1 PERF_NOTEBOOK_DRY_RUN=1 npm run perf:notebook-baseline
```
