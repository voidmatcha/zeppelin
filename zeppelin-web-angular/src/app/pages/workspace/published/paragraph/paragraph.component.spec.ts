...
describe('Published React cleared result visibility', () => {
  it('keeps a cleared earlier slot hidden when output resumes at a later index', () => {
    const { component } = setup(true);
    const note = snapshot('note-A', 'RUNNING', 'initial');
    note.note.paragraphs[0].results = {
      msg: [
        { type: DatasetType.TABLE, data: 'a\n1' },
        { type: DatasetType.TABLE, data: 'b\n2' }
      ]
    };
    component.getNote(note);
    const cleared = snapshot('note-A', 'RUNNING', '').note.paragraphs[0];
    cleared.results = undefined;
    component.paragraphData({ noteId: 'note-A', paragraph: cleared, outputCleared: true, preserveOutputTypes: true });
    expect(component.reactProps.results).toEqual([
      { type: DatasetType.TABLE, data: '' },
      { type: DatasetType.TABLE, data: '' }
    ]);
    expect(component.reactProps.hiddenResults).toEqual([true, true]);
    component.onParagraphAppendOutput({ noteId: 'note-A', paragraphId: 'same-paragraph', index: 1, data: 'b\n3' });
    expect(component.reactProps.results).toEqual([
      { type: DatasetType.TABLE, data: '' },
      { type: DatasetType.TABLE, data: 'b\n3' }
    ]);
    expect(component.reactProps.hiddenResults).toEqual([true, false]);
    append(component, 'note-A', 'a\n4');
    expect(component.reactProps.hiddenResults).toEqual([false, false]);

    component.paragraphData({ noteId: 'note-A', paragraph: cleared, outputCleared: true, preserveOutputTypes: true });
    const finished = snapshot('note-A', 'FINISHED', 'final').note.paragraphs[0];
    finished.results = { msg: [{ type: DatasetType.TABLE, data: 'a\n5' }] };
    component.paragraphData({ noteId: 'note-A', paragraph: finished });
    expect(component.reactProps.results).toEqual([{ type: DatasetType.TABLE, data: 'a\n5' }]);
    expect(component.reactProps.hiddenResults).toEqual([false]);
  });

  it('hides cleared slots restored from a running checkpoint', () => {
    const { component } = setup(true);
    component.getNote(snapshot('note-A', 'RUNNING', ''));
    expect(component.reactProps.hiddenResults).toEqual([true]);
    append(component, 'note-A', 'new output');
    expect(component.reactProps.hiddenResults).toEqual([false]);
  });
});
