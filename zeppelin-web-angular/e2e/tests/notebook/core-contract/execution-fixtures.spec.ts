...
const streamingSetting = 'zeppelin.websocket.paragraph_status_progress.enable';

type CaptureIsolation = Record<'logs' | 'notebook' | 'pid' | 'recovery' | 'root' | 'searchIndex', string>;

interface CaptureServerSettings {
  authentication: string;
  buildManifestId: string | null;
  configuration: Record<string, boolean | number | string>;
  isolation: CaptureIsolation;
  mode: string;
  port: number;
}

const readCaptureServerSettings = (): CaptureServerSettings => {
  const captureRoot = process.env.ZEPPELIN_E2E_CAPTURE_ROOT;
  if (!captureRoot) throw new Error('ZEPPELIN_E2E_CAPTURE_ROOT is required for live fixture capture');
  const settingsPath = resolve(captureRoot, 'capture-provenance.json');
  if (!existsSync(settingsPath)) {
    throw new Error(`${settingsPath} is missing; start the capture server with capture-server.sh for this root`);
  }
  const settings = JSON.parse(readFileSync(settingsPath, 'utf8')) as CaptureServerSettings;
  if (settings.isolation?.root !== realpathSync(captureRoot)) {
    throw new Error(`${settingsPath} was written for ${settings.isolation?.root}, not ${captureRoot}`);
  }
  return settings;
};

const captureProvenance = (page: Page, browserName: string, browserVersion: string, streaming: boolean) => {
  ...
  const server = readCaptureServerSettings();
  if (!server.buildManifestId) {
    throw new Error('capture server was not started with a verified --build-manifest');
  }
  if (manifest.manifestId !== server.buildManifestId) {
    throw new Error(
      `build manifest ${manifest.manifestId} does not match the capture server's verified manifest ${server.buildManifestId}`
    );
  }
  if (server.configuration?.[streamingSetting] !== streaming) {
    throw new Error(
      `capture server ${streamingSetting}=${server.configuration?.[streamingSetting]}, expected ${streaming}`
    );
  }
  const origin = new URL(page.url());
  if (Number(origin.port) !== server.port) {
    throw new Error(`capture origin ${origin.origin} does not use capture server port ${server.port}`);
  }
  const captureRoot = server.isolation.root;
  const isolation = Object.fromEntries(
    Object.entries(server.isolation).map(([name, directory]) => [
      name,
      directory === captureRoot || directory.startsWith(`${captureRoot}/`)
        ? `<capture-root>${directory.slice(captureRoot.length)}`
        : directory
    ])
  ) as CaptureIsolation;
  return {
...
const captureExecution = async (
  ...
  const recorder = createNotebookTransportRecorder({
    ...
    provenance: captureProvenance(page, browserName, browserVersion, streaming),
    scenario: cancel
  ...
  try {
    ...
    const [stopResult, closeResult] = await Promise.allSettled([recorder.stop(), recorderPage.close()]);
    if (!captureFailed && stopResult.status === 'rejected') throw stopResult.reason;
    if (!captureFailed && closeResult.status === 'rejected') throw closeResult.reason;
  }
};
