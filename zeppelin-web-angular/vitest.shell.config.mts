...
export default defineConfig({
  ...
  test: {
    ...
    coverage: {
      ...
      // No `include`: v4 then reports only files loaded by specs. Naming a directory makes the coverage provider parse the whole tree separately from the test transform, so re-measure before widening. The percentage is therefore not whole-tree coverage; see AGENTS.md.
